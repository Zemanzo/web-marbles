...
DiscordManager.prototype.initialize = function(db) {
	const self = this;
	// Set up clients
	this.db = db;
...
DiscordManager.prototype.stop = function() {
	this.chatWebhook.destroy();
	return this.client.destroy();
};

DiscordManager.prototype.authorizeClient = function(req, res) {
	let rejectionHasFired = false;
	let onReject = function() {
		if (!rejectionHasFired) {
			rejectionHasFired = true;

			// Something went wrong, so display the error page.
			res.render("chat", {
...
DiscordManager.prototype.refreshClient = function(req, res) {
